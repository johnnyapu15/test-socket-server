...
function route(url, res) {
    try {
        var buf = fs_1.readFileSync("./test/client" + url);
...
function handler(req, res) {
    console.log(req.url);
    if (req.url === '/') {
        var buf = fs_1.readFileSync('./test/client/test.html');
        res.writeHead(200, { 'Content-Type': 'text/html', 'Content-Length': buf.length });
        res.write(buf);
    }
    else {
...
var server = http_1.createServer(handler);
...
var roomNsp = namespace_1.initRoomNsp(io);
var userNsp = namespace_1.initUserNsp(io);
var positionNsp = namespace_1.initPositionNsp(io);
